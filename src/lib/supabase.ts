...
export const DEFAULT_SUPABASE_URL = 'https://ajndysndfrnsgkkknguy.supabase.co';

export const SUPABASE_URL = getEnvVar('VITE_SUPABASE_URL', localStorage.getItem('PROMOJA_SUPABASE_URL') || DEFAULT_SUPABASE_URL);
...
export const isSupabaseConfigured = (): boolean => {
  ...
  return Boolean(url && key && url.includes('supabase.co') && !key.includes('dummy'));
};

export const getSupabaseClient = () => {
  const url = localStorage.getItem('PROMOJA_SUPABASE_URL') || SUPABASE_URL || DEFAULT_SUPABASE_URL;
